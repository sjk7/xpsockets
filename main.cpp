...

    {
        try {
            ...
            xp::ConnectingSocket consock("my connecting socket",
                                         ...
                                         &myctx);
            cout << "Sending the following request: " << xp::simple_http_request << endl;
            xp::ioresult_t send_result = consock.send(xp::simple_http_request);
            assert(send_result.bytes_transferred == xp::simple_http_request.size());
            std::string data;

            int my_result = 0;
            auto read_result = consock.read2(data, [&](auto read_result, auto &d) {
                if (read_result == 0) {
                    const auto f = (d.find("\r\n\r\n"));
                    my_result = (int) f;
                    return (int) f;
            ...
            cout << "Reading data from remote server took: " << myctx.ctr << " ms." << endl;
        ...
        {
            int new_data(int read_result, std::string &data) noexcept override
            {
                if (read_result == 0) {
        ...
        try {
            ...
            xp::ConnectingSocket consock("my connecting socket", xp::endpoint_t{"google.com", 80});
            cout << "Sending the following request: " << xp::simple_http_request << endl;
            auto send_result = consock.send(xp::simple_http_request);
            assert(send_result.bytes_transferred == xp::simple_http_request.length());
            std::string data;
